...
uint8_t cBLE::disconnect(void)
{
  ...
  ch = BleList.ConnectedIndex;

	if( ch < BleList.Count && bConnected == true)
	{
    setup();    
    err_code = send_cmd("AT", strRet, 100);
		if( err_code == BLE_OK )
...
/*---------------------------------------------------------------------------
     TITLE   : connectFromLastConnect
     WORK    : 
     ARG     : void
     RET     : void
---------------------------------------------------------------------------*/
uint8_t cBLE::connectFromLastConnect(void)
{
	uint8_t err_code = BLE_OK;
	String strRet;
	uint8_t ch;
	...
	if( ch < BleList.Count )
	{
		send_cmd("AT", strRet, 100);
		send_cmd("AT+SHOW1", strRet, 100);
		send_cmd("AT+NAME", strRet, 100);
...
uint8_t cBLE::save_list( void )
{
	...
	BleListEEPROM.ConnectedIndex = BleList.ConnectedIndex;
...
uint8_t cBLE::load_list( void )
{
	...
	else
	{
		...
		BleList.ConnectedIndex = BleListEEPROM.ConnectedIndex; 
